...
/**
 * Created by zhuguangsheng on 2018/9/13.
 */
...
public class FTPClientFunctions {
    ...
    public boolean ftpUpload(String srcFilePath, String desFileName, String desDirectory) {
        ...
        try {
            if(!TextUtils.isEmpty(desDirectory)){
                boolean changeOk = this.ftpChangeDir(desDirectory);
                //如果没有目录就尝试创建
                if(!changeOk){
                    boolean mkdirOk = this.ftpMakeDir(desDirectory);
                    if(!mkdirOk){
                        status = false;
                        return status;
                    }
                    this.ftpChangeDir(desDirectory);
                }
            }
    ...
    public boolean ftpMakeDir(String path) {
        boolean status = false;
        try {
            status = ftpClient.makeDirectory(path);
        } catch (Exception e) {
            e.printStackTrace();
